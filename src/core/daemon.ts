 ...
 */
export function spawnDaemon(startArgs: string[]): void {
  ...
  const child = spawn(
    process.execPath,
    [scriptPath, 'start', ...startArgs.filter(a => a !== '--daemon' && a !== '-d')],
    {
      detached: true,
 ...
 */
export function startStatusWriter(source: HeartbeatSource): () => void {
